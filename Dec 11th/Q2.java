...
class Q2 {
    ...
    public static void applyRule(int x, int y) {
        ...
        if (occupiedCount == 0 && getChar(x, y) == 'L')
            newGrid.get(y)[x] = '#';
        else if (occupiedCount >= 5 && getChar(x, y) == '#')
            newGrid.get(y)[x] = 'L';
        else
    ...
    public static int adajacentOccupiedSeats(int x, int y) {
        ...
        for (int i = -1; i <= 1; i++) {
            for (int j = -1; j <= 1; j++) {
                if (i == 0 && j == 0)
                    continue;
                else if (getVisibleSeat(x, y, i, j) == '#')
                    occupiedCount++;
            }
        }
    ...
    public static char getVisibleSeat(int x, int y, int dirX, int dirY) {
        if (y + dirY >= grid.size() || y + dirY < 0 || x + dirX >= grid.get(0).length || x + dirX < 0)
            return '.';
        else if (getChar(x + dirX, y + dirY) == '.')
            return getVisibleSeat(x + dirX, y + dirY, dirX, dirY);
        else {
            return getChar(x + dirX, y + dirY);
        }
    }
